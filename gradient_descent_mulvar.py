...
# definition of the columns is in the link: https://archive.ics.uci.edu/ml/machine-learning-databases/housing/housing.names 
...
#predictors_scale: Array -> Array
#Input: Predictors is a feature used to perform regression
#Returns: The scaled array of all the features used
def predictors_scale(predictors):
	mu = predictors.mean()
	sigma = predictors.std()
	predictor = (predictors - mu)/sigma
	return predictor
...
def calc_rsquared(predictors, theta_fin):
	...
	return r_squared, numerator, denominator

predictors_arr = predictors_scale(predictors_ini) #Scaling all the predictors for faster processing
predictors_arr['ones'] = numpy.ones(m) #Adding ones to get the y intercept
predictors = numpy.array(predictors_arr) #Converting DataFrame to array
theta = numpy.zeros(len(predictors_arr.columns)) #Interpretation: theta holds the initial value of theta	
...
r_squared, numerator, denominator = calc_rsquared(predictors, theta_fin)
